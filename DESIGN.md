...
Orbion's design tokens are mapped to shadcn/ui CSS variables so that shadcn
components inherit the Orbion dark navy/lime palette by default. The mapping
lives in `src/renderer/src/globals.css` under `@theme inline`.

| Orbion Token | shadcn Variable | Notes |
|---|---|---|
| `--bg-panel` | `--color-background` | Main panel surface |
| `--text-primary` | `--color-foreground` | Primary text |
| `--bg-elevated` | `--color-card` | Card/elevated surface |
| `--bg-elevated` | `--color-popover` | Popover surface |
| `--accent` | `--color-primary` | Lime green accent (default actions) |
| `--accent-ink` | `--color-primary-foreground` | Text on accent fills |
| `--bg-input` | `--color-secondary` | Secondary surface (inputs) |
| `--text-secondary` | `--color-secondary-foreground` | Secondary text |
| `--bg-hover` | `--color-muted` | Muted/hover surface |
| `--text-muted` | `--color-muted-foreground` | Muted text |
| `--accent-hover` | `--color-accent` | Accent hover state |
| `--accent-ink` | `--color-accent-foreground` | Text on accent hover |
| `--danger` | `--color-destructive` | Destructive/error color |
| `--border` | `--color-border` | Standard border |
| `--bg-input` | `--color-input` | Input background |
| `--border-subtle` | `--color-ring` | Focus ring color |
| `--bg-sidebar` | `--color-sidebar-background` | Sidebar surface |
| `--text-secondary` | `--color-sidebar-foreground` | Sidebar text |
| `--bg-hover` | `--color-sidebar-accent` | Sidebar hover/accent |

Radii are mapped 1:1: `--radius-sm` through `--radius-xl` carry over from the
Orbion scale. Chart colors map to the status palette.

<!-- Last updated: 2026-07-22T14:00:00Z -->
